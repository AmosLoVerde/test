...
<br>
Per questo motivo, se si trovasse un algoritmo deterministico in tempo polinomiale anche per un solo problema $\textit{NP-c}$, allora tutti i problemi di $\textit{NP}$ diventerebbero risolvibili in tempo polinomiale, e quindi seguirebbe $P=\textit{NP}$. Viceversa, se $P\neq \textit{NP}$, allora nessun problema $\textit{NP-c}$ può appartenere a $P$.
