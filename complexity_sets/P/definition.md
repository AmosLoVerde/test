...
<br>
La classe $P$ contiene i problemi decisionali che possono essere risolti da un algoritmo deterministico in tempo polinomiale rispetto alla lunghezza dell'input.
...
* $A$ è un problema decisionale, cioè una funzione $A : I(A) \to \lbrace \text{yes, no}\rbrace$.
* $I(A)$ è l'insieme delle istanze ammissibili del problema $A$.
* $x$ è una generica istanza di input.
* $|x|$ è la lunghezza della codifica dell'istanza $x$, anche detta la sua <b>taglia</b>.
* $M_A$ è un algoritmo deterministico che decide $A$, cioè termina sempre su ogni input $x$ e restituisce la risposta corretta.
<br>
...
$$\text{DTIME}(f(n)) = \lbrace \mathbb{A}\text{ | } \mathbb{A} \text{ è un problema decisionale decidibile da un algoritmo deterministico in tempo } O(f(n))\rbrace$$
</div>
Quindi:
$$
